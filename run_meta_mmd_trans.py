	...
	for seed in range(990, 990+args.trial_num):
		...
		def train(epoch):
			...
			for batch in tqdm.tqdm(range(len(fea_train_generated) // train_batch_size), desc="Traning for deep MMD"):
				...
				TEMP = MMDu(outputs, inputs.shape[0], X.view(X.shape[0],-1), sigma, sigma0_u, ep, coeff_xy=args.coeff_xy, is_yy_zero=args.is_yy_zero, is_xx_zero=args.is_xx_zero)
		...
		if not args.test_flag:
			for epoch in range(start_epoch, start_epoch+epochs):
				...
				if (epoch+1)%1==0:
					...
					auroc_value_epoch = test(epoch, fea_real= val_sing_real, fea_generated= val_sing_generated,test_flag = True)
			...
			for i in range(num_target):
				two_sample_test(0, fea_real_ls=fea_real[i*test_lenth:(i+1)*test_lenth], fea_generated_ls=fea_generated[i*test_lenth:(i+1)*test_lenth],test_flag = True,N=10)
